...
class MockOpenAIClient:
    def chat_completions_create(self, messages, model="gpt-3.5-turbo", temperature=0.7):
        return type('obj', (object,), {
            'choices': [type('obj', (object,), {
                'message': type('obj', (object,), {
                    'content': '{"adjusted_scores": [0.95, 0.88, 0.82, 0.91, 0.75], "reasoning": "Boosted sci-fi and thriller movies based on recent viewing patterns and evening context."}'
                })()
            })()]
        })()
...
class HybridPersonalizationSystem:
    ...
    @classmethod
    def load_model(cls, save_dir="./saved_model", openai_api_key=None):
        ...
        system = cls(openai_api_key=openai_api_key)
        ...
        try:
            ...
            for i, (movie_id, score) in enumerate(zip(deep_recommendations, adjusted_scores)):
                final_recommendations.append({
                    ...
                    'final_score': (0.8 - (i * 0.1)) * 0.6 + score * 0.4  # Weighted combination
                })
...
def create_sample_data():
    ...
    print("Creating sample MovieLens-style data...")
    
    np.random.seed(42)
    users = range(1, 1001)  # 1000 users
    movies = range(1, 501)   # 500 movies
    
    ratings_data = []
    for _ in range(5000):  # 5000 ratings
        user = np.random.choice(users)
        movie = np.random.choice(movies)
        rating = np.random.choice([1, 2, 3, 4, 5], p=[0.1, 0.1, 0.2, 0.3, 0.3])
        ratings_data.append({'userId': user, 'movieId': movie, 'rating': rating})
    
    ratings_df = pd.DataFrame(ratings_data).drop_duplicates(['userId', 'movieId'])
...
def train_and_save_model():
    ...
    # Initialize system
    system = HybridPersonalizationSystem()
...
def load_and_test_model():
    """Inference script - load saved model and test recommendations"""
    print("=== INFERENCE SCRIPT ===")
    
    # Load the model
    system = HybridPersonalizationSystem.load_model(save_dir="./movie_recommender_model")
...
if __name__ == "__main__":
    ...
    # Uncomment to load and test model
    # load_and_test_model()
    
    print("Scripts ready! Run train_and_save_model() or load_and_test_model()")